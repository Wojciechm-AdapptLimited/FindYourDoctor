...
public class DiseaseService
{
    ...
    public List<Doctor> GetRecommendedDoctors(HashSet<string> symptomNames)
    {
        var symptoms = Symptoms.Where(x => symptomNames.Contains(x.Name)).ToList();
        var diseases = GetPossibleDiseases(symptoms);
        return diseases.SelectMany(d => d.Specialization.Doctors).DistinctBy(x => x.PwzNumber).ToList();
    }
    ...
    private IEnumerable<Disease> GetPossibleDiseases(IEnumerable<Symptom> symptoms)
    {
        var diseases = _context.Diseases.Include(d => d.Specialization).ThenInclude(s => s.Doctors).ToList();

        foreach (var symptom in symptoms)
        {
            var list = _context.Diseases.FromSql($"SELECT * FROM public.get_diseases_for_symptom({symptom.SymptomId})").ToList();
            diseases = diseases.IntersectBy(list, d => d).ToList();
        }
